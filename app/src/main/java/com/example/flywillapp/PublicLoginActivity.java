    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSignUp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent i=new Intent(PublicLoginActivity.this,PublicSignupActivity.class);
                startActivity(i);
        ...
        mSignIn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                final String email= mEmail.getText().toString();
                final String password= mPassword.getText().toString();
                mAuth.signInWithEmailAndPassword(email,password).addOnCompleteListener(PublicLoginActivity.this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (!task.isSuccessful()) {
                            Toast.makeText(PublicLoginActivity.this, "sign in error", Toast.LENGTH_SHORT).show();
                        }
                        else {


                            DatabaseReference reference=FirebaseDatabase.getInstance().getReference().child("Users").child("Public").child(mAuth.getCurrentUser().getUid());
                            reference.addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    if(dataSnapshot.exists())
                                    {
                                        Intent intent=new Intent(PublicLoginActivity.this,PublicMapActivity.class);
                                        startActivity(intent);
                                        finish();
                                    }
                                    else
                                    {
                                        Toast.makeText(PublicLoginActivity.this,"Signin Error",Toast.LENGTH_SHORT).show();
